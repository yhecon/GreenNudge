...
*****************************************************************************
* 		              PART B. Regresssion: Table-S12 					*
*****************************************************************************
* whether plant before
gen ptb = (m_plant_tree_cnt_std>0)
gen ptb_t = ptb*t
gen mptcs_t = m_plant_tree_cnt_std*t
replace mptcs_t=0 if mptcs_t==.

reghdfe p_e_ t if ptb == 1,  absorb(id time_order) vce(cl id city_month)
sum p_e_ if ptb==1 & t == 0

reghdfe p_e_ t if ptb == 0,  absorb(id time_order) vce(cl id city_month)
sum p_e_ if ptb==0 & t == 0

reghdfe p_e_ t ptb_t,  absorb(id time_order) vce(cl id city_month)
